...
export const VIVIM_IDENTITY: VIVIMIdentity = {
  ...
  capabilities: [
    {
      id: 'capture',
      name: 'Capture',
      description: 'Extract conversations from AI platforms',
      available: true,
    },
    {
      id: 'vault',
      name: 'Vault',
      description: 'Encrypted personal knowledge store',
      available: true,
    },
    { id: 'byok', name: 'BYOK Chat', description: 'Bring your own AI keys', available: true },
    {
      id: 'social',
      name: 'Social Feed',
      description: 'Share and discover conversations',
      available: true,
    },
    {
      id: 'acu',
      name: 'ACU System',
      description: 'Atomic Chat Units for modular knowledge',
      available: true,
    },
  ...
  privacyFeatures: [
    {
      ...
      description: 'Only you can read your vault',
      enabled: true,
    },
    {
      id: 'zk',
      name: 'Zero-Knowledge Sync',
      description: 'Servers cannot read your data',
      enabled: true,
    },
    {
      id: 'local',
      name: 'Local-First',
      description: 'Data in browser IndexedDB first',
      enabled: true,
    },
    {
      id: 'no-training',
      name: 'No AI Training',
      description: 'Your data is never used to train AI',
      enabled: true,
    },
    {
      id: 'did',
      name: 'DID Identity',
      description: 'Self-sovereign decentralized identity',
      enabled: false,
    },
  ],
  ...
  layers: [
    ...
    { id: 'L3', name: 'Topic Context', priority: 80, description: 'Current topic being discussed' },
    {
      ...
      name: 'Entity Context',
      priority: 70,
      description: 'People and projects mentioned',
    },
  ...
  faq: [
    ...
    },
    {
      ...
      answer:
        "Yes. VIVIM uses end-to-end encryption meaning only you can read your vault. Your API keys are encrypted with your master key and never touch our servers. We can't read your conversations even if we wanted to.",
      category: 'privacy',
    },
    {
    ...
    },
    {
      ...
      answer:
        'VIVIM has a free tier with limited captures and storage. Paid tiers add more captures, storage, and advanced features. BYOK uses your own AI API keys - you pay the AI providers directly.',
      category: 'billing',
    },
  ],
...
### Capture From:
${VIVIM_IDENTITY.providers
  .filter((p) => p.captureSupported)
  .map((p) => `- ${p.name} (${p.domain})`)
  .join('\n')}

### BYOK (Bring Your Own Key) - Use YOUR own API key:
${VIVIM_IDENTITY.providers
  .filter((p) => p.byokSupported)
  .map((p) => `- ${p.name}`)
  .join('\n')}

## Privacy & Security

${VIVIM_IDENTITY.privacyFeatures
  .filter((f) => f.enabled)
...
export function findFAQ(keyword: string): VIVIMFAQ | undefined {
  ...
  return VIVIM_IDENTITY.faq.find(
...
export function isCaptureSupported(providerDomain: string): boolean {
  return VIVIM_IDENTITY.providers.some(
    (p) =>
      p.domain.includes(providerDomain.toLowerCase()) || p.id.includes(providerDomain.toLowerCase())
  );
}
...
export function isBYOKSupported(providerId: string): boolean {
  return VIVIM_IDENTITY.providers.some((p) => p.id === providerId.toLowerCase() && p.byokSupported);
}
...
export function getSimpleExplanation(topic: string): string {
  const explanations: Record<string, string> = {
    'what-is-vivim':
      'VIVIM is your personal AI conversation manager - like iPhotos for your AI chats. We capture conversations from any AI (ChatGPT, Claude, Gemini, etc.), store them privately in your encrypted vault, and let you continue or share them.',
    privacy:
      'VIVIM uses end-to-end encryption so only you can read your vault. Your API keys are encrypted with your master key and never touch our servers. Zero-knowledge means we cant read your data even if we wanted to.',
    capture:
      'Just paste a link from any supported AI (ChatGPT, Claude, Gemini, Grok, DeepSeek, Kimi, Qwen, z.ai, Mistral) and VIVIM extracts the full conversation including text, code, images, and tables.',
    ...
    circles:
      'Circles are groups you share with. Create circles for work, friends, or specific projects. Control exactly who sees what.',
    vault:
      'Your vault is your private, encrypted knowledge store. It lives locally in your browser first, then syncs encrypted to the cloud. You own it, we just protect it.',
  };
