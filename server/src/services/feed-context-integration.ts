...
export async function processFeedEngagement(event: FeedEngagementEvent): Promise<{
  ...
  try {
    ...
    operationLog.debug({ msg: 'Updating topic affinities', topicCount: contentDetails.topicSlugs.length });
    const topicsUpdated = await updateTopicAffinities(event.userId, contentDetails.topicSlugs, weight);

    operationLog.debug({ msg: 'Updating entity affinities', entityCount: contentDetails.entityIds.length });
    const entitiesUpdated = await updateEntityAffinities(event.userId, contentDetails.entityIds, weight);
    ...
    if (contentType === 'acu' || contentType === 'conversation') {
      const conversation = await prisma.conversation.findFirst({
        where: {
          OR: [
            { id: contentId },
            { atomicChatUnits: { some: { id: contentId } } }
          ]
        },
        include: {
...
async function updateTopicAffinities(userId: string, topicSlugs: string[], weight: number): Promise<TopicAffinityUpdate[]> {
  const operationLog = createOperationLogger('updateTopicAffinities', { userId, topicSlugs, weight });
  ...
  for (const slug of topicSlugs) {
    try {
      ...
      operationLog.error({ msg: 'Failed to update topic affinity', slug, error: (error as Error).message });
...
async function updateEntityAffinities(userId: string, entityIds: string[], weight: number): Promise<EntityAffinityUpdate[]> {
  const operationLog = createOperationLogger('updateEntityAffinities', { userId, entityIds, weight });
  ...
  for (const entityId of entityIds) {
    try {
      ...
      operationLog.error({ msg: 'Failed to update entity affinity', entityId, error: (error as Error).message });
    }
  }
...
async function invalidateRelevantBundles(userId: string, topicSlugs: string[], entityIds: string[]): Promise<string[]> {
  const operationLog = createOperationLogger('invalidateRelevantBundles', { userId, topicSlugs, entityIds });
...
async function triggerMemoryExtraction(userId: string, contentId: string, contentType: string): Promise<void> {
  const operationLog = createOperationLogger('triggerMemoryExtraction', { userId, contentId, contentType });
  ...
  try {
    ...
    operationLog.error({ msg: 'Failed to trigger memory extraction', error: (error as Error).message });
  }
}

export async function generateContextualFeed(userId: string, options: {
  limit?: number;
  offset?: number;
  ...
  try {
    ...
    return {
      success: true,
      items: boostedItems,
      contextBoost: includeContextBoost ? { activeTopics, activeEntities, boostedTopics: boostedTopicSlugs } : undefined
    };
  } catch (error) {
    operationLog.error({ msg: 'Failed to generate contextual feed', error: (error as Error).message });
    await serverErrorReporter.reportServerError(
  ...
  try {
    ...
    return {
      ...
      state: {
        ...
        activeEntities: entities.map(e => ({ id: e.id, name: e.name, affinity: e.affinity ?? 0.5 })),
        recentEngagement: recentEngagement.map(e => ({ contentId: e.contentId, action: e.action, timestamp: e.createdAt })),
...
export async function processEngagementBatch(events: FeedEngagementEvent[]): Promise<{
  ...
  const operationLog = createOperationLogger('processEngagementBatch', { batchSize: events.length });
