 ...
 * Tiered Import Types
 * 
 * Type definitions for the tiered import system
 */
...
export type ImportConversationState = 
  | 'PENDING'
  | 'VALIDATING'
  | 'STORED'
  | 'ACU_GENERATED'
...
export interface ImportScanResult {
  ...
  conversationSizeDistribution: {
    small: number;    // < 5 messages
    medium: number;   // 5-20 messages
    large: number;   // 20-100 messages
    xlarge: number;  // > 100 messages
  };
...
export interface ImportJobProgress {
  ...
  tiers: Record<ImportTier, TierProgress>;
  fileName: string;
  totalConversations: number;
